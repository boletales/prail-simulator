...
var monadMaybe = { Applicative0: () => applicativeMaybe, Bind1: () => bindMaybe };
...
// output-es/Internal.Layout.Operation/index.js
...
var foldM2 = (f) => (b0) => foldlArray((b) => (a) => {
  if (b.tag === "Just") {
    return f(b._1)(a);
  }
  if (b.tag === "Nothing") {
...
var setManualStop = (v) => (nodeid) => (jointid) => (stop) => {
  ...
  if ($0.tag === "Just") {
    ...
    if ($1.tag === "Just") {
      ...
      if ($2.tag === "Just") {
        const $3 = updateRailNodeAt({ ...$0._1, signals: $2._1 })(nodeid)(v.rails);
        if ($3.tag === "Just") {
          return { ...v, rails: $3._1 };
        }
        if ($3.tag === "Nothing") {
...
var removeSignal = (v) => (nodeid) => (jointid) => updateSignalRoutes({
  ...
  rails: (() => {
    const $0 = modifyRailNode((v2) => ({ ...v2, signals: filterImpl((v4) => v4.jointid !== jointid, v2.signals), invalidRoutes: filterImpl((v4) => v4.jointid !== jointid, v2.invalidRoutes) }))(nodeid)(v.rails);
    if ($0.tag === "Nothing") {
      return v.rails;
    }
    if ($0.tag === "Just") {
...
var addTrainset = (v) => (nodeid) => (jointid) => (types) => {
  const go = (go$a0$copy) => (go$a1$copy) => (go$a2$copy) => (go$a3$copy) => {
    ...
    while (go$c) {
      ...
      if ($02.tag === "Just") {
        ...
        if (lenhere < len) {
          const $1 = find((c) => c.from === info.newjoint)($02._1.connections);
          if ($1.tag === "Just") {
            go$a0 = [{ nodeid: nid, jointid: jid, railinstance: $02._1, shapes: info.shapes, length: lenhere }, ...rs];
            go$a1 = $1._1.nodeid;
            go$a2 = $1._1.jointid;
            go$a3 = len - lenhere;
            continue;
        ...
        go$r = $Maybe(
          ...
          {
            types,
            route: [{ nodeid: nid, jointid: jid, railinstance: $02._1, shapes: info.shapes, length: lenhere }, ...rs],
  ...
  if ($0.tag === "Just") {
    ...
    if ($1.tag === "Just") {
      const $2 = go([])($1._1.nodeid)($1._1.jointid)(toNumber(types.length) * 0.5140186915887851 - 0.04672897196261683);
      if ($2.tag === "Just") {
        return { ...v, traincount: v.traincount + 1 | 0, trains: [...v.trains, $2._1] };
      }
      if ($2.tag === "Nothing") {
        return v;
      }
...
var addSignal = (v) => (nodeid) => (jointid) => {
  ...
  if ($0.tag === "Just") {
    ...
    const $1 = modifyRailNode((v2) => ({ ...v2, signals: [...v2.signals, signal] }))(nodeid)(v.rails);
    if ($1.tag === "Just") {
...
var addRailWithPos = (v) => (v1) => (pos) => {
  ...
  if (foldlArray(boolConj)(true)(arrayMap((v2) => {
    ...
    const newnodeId = v.instancecount;
    return $Maybe(
      "Just",
...
var addInvalidRoute = (v) => (nodeid) => (jointid) => {
  ...
  if ($0.tag === "Just") {
    ...
    const $1 = modifyRailNode((v2) => ({ ...v2, invalidRoutes: [...v2.invalidRoutes, { nodeid, jointid }] }))(nodeid)(v.rails);
    if ($1.tag === "Just") {
      return updateSignalRoutes({ ...v, updatecount: v.updatecount + 1 | 0, rails: $1._1 });
    }
    if ($1.tag === "Nothing") {
...
var serialAll3 = /* @__PURE__ */ serialAll(intSerialize4);
...
var serialAll4 = /* @__PURE__ */ serialAll(intSerialize8);
var intSerialize9 = {
  fromSerial: (i) => {
    const $0 = intSerializeSum1.fromSerial(i);
    if ($0.tag === "Just") {
...
var serialAll5 = /* @__PURE__ */ serialAll(intSerialize9);
...
var serialAll6 = /* @__PURE__ */ serialAll(intSerialize10);
...
var serialAll7 = /* @__PURE__ */ serialAll(intSerialize11);
var intSerialize12 = {
  fromSerial: (i) => {
...
var crossoverLRailGen = (len) => (name) => {
  const poe = { coord: { x: 1, y: 0, z: 0 }, angle: toNumber(0) * 6.283185307179586 / toNumber(8), isPlus: false };
  const pob = { coord: { x: 1 - len, y: 0, z: 0 }, angle: toNumber(4) * 6.283185307179586 / toNumber(8), isPlus: true };
  const ro = [{ start: pob, end: poe, length: partLength(pob)(poe) }];
  const pie = { coord: { x: 0, y: -0.28037383177570097, z: 0 }, angle: toNumber(4) * 6.283185307179586 / toNumber(8), isPlus: true };
  const rn = slipShapes()({ start: pie, end: poe });
  const pib = { coord: { x: len, y: -0.28037383177570097, z: 0 }, angle: toNumber(0) * 6.283185307179586 / toNumber(8), isPlus: false };
  const ri = [{ start: pib, end: pie, length: partLength(pib)(pie) }];
...
var crossoverShortLRail = /* @__PURE__ */ crossoverLRailGen(0.5)("short");
var crossoverShortRRail = /* @__PURE__ */ memorizeRail(/* @__PURE__ */ flipRail_(crossoverShortLRail));
var crossoverLRail = /* @__PURE__ */ crossoverLRailGen(1)("");
...
var halfScissorsLRail = /* @__PURE__ */ (() => {
  const pm = { coord: { x: 1, y: 0, z: 0 }, angle: toNumber(0) * 6.283185307179586 / toNumber(8), isPlus: true };
  const pe = { coord: { x: 0, y: 0, z: 0 }, angle: toNumber(4) * 6.283185307179586 / toNumber(8), isPlus: false };
  const r1 = slipShapes()({ start: pe, end: pm });
  const pp = { coord: { x: 0.5, y: 0.14018691588785048, z: 0 }, angle: calcMidAngle2(1)(0.28037383177570097), isPlus: true };
  const r0 = [{ start: pe, end: pp, length: partLength(pe)(pp) }];
  return memorizeRail(toRail_(intSerialize10)(intSerialize1)({
    name: "halfscissors",
    flipped: false,
    opposed: false,
    ...
    if (v.version <= 2) {
      ...
      return {
        ...
        trains: arrayMap((v3) => ({
          ...v3,
          route: arrayMap((v4) => {
            const newId = findInstanceId(v4.nodeid);
            return {
              ...v4,
              nodeid: newId,
              railinstance: (() => {
...
// output-es/Internal.Layout.DrawInfo/index.js
...
var sum3 = /* @__PURE__ */ foldlArray(numAdd)(0);
var max4 = (x) => (y) => {
  const v = ordNumber.compare(x)(y);
  if (v === "LT") {
    return y;
...
var calcAcceralation = (notch) => (speed) => -speed * speed * 1e-3 + (() => {
  ...
  if (notch > 0) {
    ...
    return 0.4 / (speed / 0.025 / 40) * max4(0)((toNumber(notch) - 0.5 - speed / 0.025 / 21) * 2 + 1);
  }
...
// output-es/Internal.Layout.Tick/index.js
var identity7 = (x) => x;
var max5 = (x) => (y) => {
  const v = ordNumber.compare(x)(y);
  if (v === "LT") {
...
var trainTick = (v) => (v1) => (dt) => {
  ...
  if (nextsignal.signal.tag === "Nothing") {
    const $0 = {
      ...v1,
      signalRestriction: max5(v1.signalRestriction)((() => {
        if (nextsignal.signal.tag === "Nothing") {
          return 0.625;
        }
    ...
    const notch = min3($0.notch)(getMaxNotchWithNextSignal(nextsignal)($0));
  ...
  if (nextsignal.signal.tag === "Just") {
    if (v1.signalRulePhase === 0) {
      ...
      const $2 = {
        ...$02.t,
        signalRestriction: max5($02.t.signalRestriction)((() => {
          if (nextsignal.signal.tag === "Nothing") {
            return 0.625;
          }
          if (nextsignal.signal.tag === "Just") {
      ...
      const notch2 = min3($2.notch)(getMaxNotchWithNextSignal(nextsignal)($2));
    ...
    if (v1.signalRulePhase === 1 && v1.speed === 0) {
      ...
      const $12 = $02.l;
      const $2 = {
        ...($02.f ? flipTrain : identity7)($02.t.signalRulePhase === 1 ? { ...$02.t, signalRulePhase: 3 } : $02.t),
        signalRestriction: max5(($02.f ? flipTrain : identity7)($02.t.signalRulePhase === 1 ? { ...$02.t, signalRulePhase: 3 } : $02.t).signalRestriction)((() => {
          if (nextsignal.signal.tag === "Nothing") {
            return 0.625;
          }
          if (nextsignal.signal.tag === "Just") {
...
var toDoubleShortRPlusRail2 = toDoubleShortRPlusRail;
var toDoubleShortLPlusRail2 = toDoubleShortLPlusRail;
...
var crossoverShortRRail2 = crossoverShortRRail;
var crossoverShortLRail2 = crossoverShortLRail;
...
export {
  ...
  crossoverShortLRail2 as crossoverShortLRail,
  crossoverShortRRail2 as crossoverShortRRail,
  ...
  toDoubleShortLPlusRail2 as toDoubleShortLPlusRail,
  toDoubleShortRPlusRail2 as toDoubleShortRPlusRail,
